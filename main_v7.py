        ...
        if not isinstance(join_pair, list):
            ...
            if initialize:
                ...
                if left_join_table in joined_table and right_join_table in joined_table:
                    ...
                    join_result = self_join(left_join_table, right_join_table, left_join_col, right_join_col, root_path,
                                            join_result, join_order_helper)
...
def join_table(joined_table, waiting_join_table, joined_table_col, waiting_join_col, root_path, join_result,
    ...
    join_result_row_num_repeatation_dict = dict()
    for i in range(len(join_result)):

        row_number = join_result[i][joined_table_index]

        if row_number not in join_result_row_num_repeatation_dict:
            join_result_row_num_repeatation_dict[row_number] = [i]

            temp_joined_lst[0].append(row_number)
            temp_joined_lst[1].append(joined_lst[row_number])

        else:
            join_result_row_num_repeatation_dict[row_number].append(i)
    ...
    if len(joined_lst[0]) < len(waiting_join_lst[0]):
        ...
        for i in range(len(joined_lst[0])):
            ...
            # if row_value in joined_lst_dict:
            #     joined_lst_dict[row_value].append(row_number)
            # else:
            #     joined_lst_dict[row_value] = [row_number]
        ...
        for i in range(len(waiting_join_lst[0])):
            ...
            if waiting_row_value in joined_lst_dict:
                ...
                for joined_row_number in joined_lst_dict[waiting_row_value]:
                    ...
                    for index in join_result_row_num_repeatation_dict[joined_row_number]:
                        tmp = list(join_result[index])
                        tmp.append(waiting_row_number)
                        temp_join_result.append(tmp)


    else:

        waiting_join_lst_dict = dict()

        for i in range(len(waiting_join_lst[0])):
            waiting_row_value = waiting_join_lst[1][i]
            waiting_row_number = waiting_join_lst[0][i]

            if waiting_row_value in waiting_join_lst_dict:
                waiting_join_lst_dict[waiting_row_value].append(waiting_row_number)
            else:
                waiting_join_lst_dict[waiting_row_value] = [waiting_row_number]

        for i in range(len(joined_lst[0])):
            joined_row_value = joined_lst[1][i]
            joined_row_number = joined_lst[0][i]

            if joined_row_value in waiting_join_lst_dict:

                for waiting_join_row_number in waiting_join_lst_dict[joined_row_value]:

                    for index in join_result_row_num_repeatation_dict[joined_row_number]:
                        temp_join_result.append(join_result[index] + [waiting_join_row_number])
...
if __name__ == "__main__":
    ...
    feature_dict = loader.execute_loader(
         "data/xxs/A.csv,data/xxs/B.csv,data/xxs/C.csv,data/xxs/D.csv,data/xxs/E.csv,data/xxs/F.csv")


    root_path = loader.get_root_path("data/xxs/A.csv,data/xxs/B.csv,data/xxs/C.csv,data/xxs/D.csv,data/xxs/E.csv,data/xxs/F.csv")
